...
class InstanceControllerWindows extends InstanceController {
  ...
  void _writePipeData(String filename, List<String>? arguments) {
    ...
    try {
      final result =
          WriteFile(pipe, bytes as Pointer<Uint8>, bytesString.length, numWritten, nullptr);
      if (result == 0) {
  ...
  static void _startReadPipeIsolate(SendPort writer) {
    final reader = ReceivePort()
      ..listen((msg) {
        if (msg is int) _readPipe(writer, msg);
      });
    ...
    try {
      while (true) {
        ...
        try {
          ...
          ReadFile(pipeHandle, data as Pointer<Uint8>, dataSize, numRead, overlap);
